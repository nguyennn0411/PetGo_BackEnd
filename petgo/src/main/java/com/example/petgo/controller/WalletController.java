...
public class WalletController {
    ...
    public ResponseEntity<Map<String, Object>> myWallet(HttpServletRequest request) {
        return ResponseEntity.ok(Map.of("message", "Lấy ví thành công.", "result", walletService.getMyWallet(request)));
    }

    @GetMapping("/transactions")
    public ResponseEntity<Map<String, Object>> myTransactions(HttpServletRequest request) {
        return ResponseEntity.ok(
                Map.of("message", "Lấy lịch sử ví thành công.", "result", walletService.getMyTransactions(request)));
    }
    ...
    public ResponseEntity<Map<String, Object>> verifyTopUp(@PathVariable Long transactionId) {
        return ResponseEntity.ok(Map.of("message", "Đồng bộ trạng thái nạp ví thành công.", "result",
                walletService.verifyTopUp(transactionId)));
    }
    ...
    public ResponseEntity<Map<String, Object>> withdraw(HttpServletRequest request,
        ...
        return ResponseEntity.status(HttpStatus.CREATED)
                .body(Map.of("message", "Tạo yêu cầu rút tiền thành công. Admin sẽ xác nhận thủ công.", "result",
