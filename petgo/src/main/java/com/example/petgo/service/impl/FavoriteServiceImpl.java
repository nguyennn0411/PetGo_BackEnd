...
public class FavoriteServiceImpl implements FavoriteService {
        ...
        public FavoriteListResponse getFavorites(Long ownerUserId, Double latitude, Double longitude) {
                ...
                List<Favorite> favorites = favoriteRepository
                                .findByUser_IdAndProvider_StatusAndProvider_DeletedAtIsNullOrderByCreatedAtDescIdDesc(
                                                ownerUserId, "ACTIVE");
                ...
                Map<Long, List<ProviderService>> servicesMap = providerServiceRepository
                                .findActiveByProviderIds(providerIds).stream()
                                .collect(Collectors.groupingBy(ps -> ps.getProvider().getId(), LinkedHashMap::new,
                                                Collectors.toList()));

                Map<Long, List<ProviderBusinessHour>> hoursMap = providerBusinessHourRepository
        ...
        private FavoriteProviderResponse mapItem(
                ...
                Double distanceKm = calculateDistanceKm(userLat, userLng, toDouble(provider.getLatitude()),
                                toDouble(provider.getLongitude()));
                ...
                List<Long> categoryIds = providerServices.stream()
                                .map(ps -> ps.getService().getCategory().getId())
                                .filter(Objects::nonNull)
                                .distinct()
                                .toList();
                List<String> categoryNames = providerServices.stream()
                                .map(ps -> ps.getService().getCategory().getName())
                                .filter(Objects::nonNull)
                                .distinct()
                                .toList();
                ...
                return FavoriteProviderResponse.builder()
                                ...
                                .savedAt(favorite.getCreatedAt() != null
                                                ? favorite.getCreatedAt().atZone(APP_ZONE).format(SAVED_AT_VIEW)
                                                : null)
                                .build();
        }
        ...
        private String resolveFeaturedService(List<ProviderService> services) {
                return services.stream()
                                .findFirst()
                                .map(service -> firstNonBlank(service.getCustomName(),
        ...
        private boolean isOpenNow(List<ProviderBusinessHour> businessHours) {
                ...
                return businessHours.stream()
                                ...
                                .filter(hour -> !currentTime.isBefore(hour.getOpensAt())
                                                && !currentTime.isAfter(hour.getClosesAt()))
                                .isPresent();
        }
        ...
        private String buildAddress(ProviderProfile provider) {
                ...
                if (provider == null)
                        return "";
                if (notBlank(provider.getPrimaryAddressLine1()))
                        parts.add(provider.getPrimaryAddressLine1().trim());
                if (notBlank(provider.getWard()))
                        parts.add(provider.getWard().trim());
                if (notBlank(provider.getDistrict()))
                        parts.add(provider.getDistrict().trim());
                if (notBlank(provider.getCity()))
                        parts.add(provider.getCity().trim());
                if (parts.isEmpty() && notBlank(provider.getProvince()))
                        parts.add(provider.getProvince().trim());
