...
public class PetServiceImpl implements PetService {
    ...
    private void applyRequest(Pet pet, PetUpsertRequest request) {
        ...
        pet.setAvatarUrl(trimToNull(request.avatarUrl()));
    ...
    private void savePhotos(Pet pet, List<String> photoUrls, String avatarUrl) {
        ...
        if (photoUrls != null) {
            for (String photoUrl : photoUrls) {
                if (photoUrl != null && !photoUrl.isBlank() && normalized.stream().noneMatch(existing -> existing.equals(photoUrl.trim()))) {
    ...
    private PetResponse mapPet(Pet pet) {
        List<PetPhotoResponse> photos = petPhotoRepository.findByPet_IdOrderByPrimaryDescSortOrderAscIdAsc(pet.getId()).stream()
        ...
        String avatar = firstNonBlank(pet.getAvatarUrl(), photos.stream().map(PetPhotoResponse::photoUrl).findFirst().orElse(null), fallbackPetAvatar(pet));
    ...
    private String generatePetCode() {
        return "PET-" + UUID.randomUUID().toString().replace("-", "").substring(0, 10).toUpperCase(Locale.ROOT);
    }
    ...
    private String fallbackPetAvatar(Pet pet) {
        String seed = firstNonBlank(pet.getName(), pet.getPetCode(), String.valueOf(pet.getId()));
        return "https://images.unsplash.com/photo-1517849845537-4d257902454a?auto=format&fit=crop&q=80&w=800&sig=" + Math.abs(Objects.hashCode(seed));
    }
