...
public class HomeServiceImpl implements HomeService {
        ...
        public HomePageResponse getHomePage(Double latitude, Double longitude) {
                ...
                List<HomeProviderResponse> mappedNearbyProviders = mapProvidersWithDistance(nearbyProviders, latitude,
                                longitude);
                List<HomeProviderResponse> mappedFeaturedProviders = mapProvidersWithDistance(featuredProviders,
                                latitude,
                                longitude);
                ...
                return HomePageResponse.builder()
                                ...
                                .stats(HomeStatsResponse.builder()
                                                ...
                                                .activeProviders(Math.max(mappedNearbyProviders.size(),
                                                                mappedFeaturedProviders.size()))
        ...
        private List<HomeProviderResponse> mapProvidersWithDistance(List<ProviderProfile> providers, Double userLat,
                ...
                return providers.stream()
                                .map(provider -> new ProviderWithDistance(provider,
                                                calculateDistanceKm(userLat, userLng, provider.getLatitude(),
        ...
        private HomeReviewResponse mapReview(Review review) {
                ...
                return HomeReviewResponse.builder()
                                .id(review.getId())
                                .customerName(
                                                review.getCustomerUser() != null
                                                                ? review.getCustomerUser().getFullName()
                                                                : "Khách hàng PetGo")
                                .petLabel(buildPetLabel(pet))
                                .rating(review.getRating())
                                .text(review.getComment())
                                .avatar(review.getCustomerUser() != null ? review.getCustomerUser().getAvatarUrl()
                                                : null)
                                .build();
        }
        ...
        private Double calculateDistanceKm(Double userLat, Double userLng, BigDecimal providerLat,
                        BigDecimal providerLng) {
                ...
                double a = Math.sin(dLat / 2) * Math.sin(dLat / 2)
                                + Math.cos(Math.toRadians(userLat))
                                                * Math.cos(Math.toRadians(providerLat.doubleValue()))
