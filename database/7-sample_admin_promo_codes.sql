...
SET @sample_bath_service_id = (
    SELECT ps.id
    FROM provider_services ps
    JOIN services s ON s.id = ps.service_id
    WHERE ps.provider_id = @sample_provider_id
      AND s.service_code IN ('SVC-DEMO-BATH-BASIC', 'SVC-SAMPLE-GROOMING')
    ORDER BY ps.id
    LIMIT 1
);
...
INSERT INTO promo_codes (
    code, name, description, owner_type, provider_id, created_by_user_id,
    ...
    user_segment, min_completed_bookings, applicable_days_of_week, provider_ids, provider_service_ids,
...
SELECT 'PETGO10K', 'Giảm 10.000đ mọi booking',
       ...
       'ADMIN', NULL, @sample_admin_id, 'PROMO_CODE', 'BOOKING', 'FIXED_AMOUNT', 10000.00, 10000.00, 0.00,
       500, 1, 0, FALSE, FALSE, 10,
       'ALL', NULL, NULL, NULL, NULL, NULL, NULL, 'Giảm 10K', '/booking',
...
SELECT 'PETGO20MAX20K', 'Giảm 20% tối đa 20.000đ',
       ...
       'ADMIN', NULL, @sample_admin_id, 'PROMO_CODE', 'BOOKING', 'PERCENTAGE', 20.00, 20000.00, 0.00,
       300, 1, 0, FALSE, FALSE, 20,
       'ALL', NULL, NULL, NULL, NULL, NULL, NULL, 'Giảm 20%', '/booking',
...
SELECT 'PETGO50K', 'Giảm 50.000đ đơn từ 300.000đ',
       ...
       'ADMIN', NULL, @sample_admin_id, 'SEASONAL', 'BOOKING', 'FIXED_AMOUNT', 50000.00, 50000.00, 300000.00,
       100, 1, 0, FALSE, FALSE, 30,
       'ALL', NULL, NULL, NULL, NULL, NULL, NULL, 'Giảm 50K', '/search',
...
SELECT 'NEWUSER10K', 'Khách mới giảm 10.000đ',
       ...
       'ADMIN', NULL, @sample_admin_id, 'FIRST_BOOKING', 'BOOKING', 'FIXED_AMOUNT', 10000.00, 10000.00, 0.00,
       300, 1, 0, FALSE, FALSE, 15,
       'NEW_USER', NULL, NULL, NULL, NULL, NULL, NULL, 'Khách mới', '/booking',
...
SELECT 'LOYAL10K', 'Khách quay lại giảm 10.000đ',
       ...
       'ADMIN', NULL, @sample_admin_id, 'LOYALTY', 'BOOKING', 'FIXED_AMOUNT', 10000.00, 10000.00, 0.00,
       300, 1, 0, FALSE, FALSE, 15,
       'RETURNING_USER', 1, NULL, NULL, NULL, NULL, NULL, 'Khách thân thiết', '/booking',
...
SELECT 'WEEKEND10K', 'Cuối tuần giảm 10.000đ',
       ...
       'ADMIN', NULL, @sample_admin_id, 'SEASONAL', 'BOOKING', 'FIXED_AMOUNT', 10000.00, 10000.00, 0.00,
       300, 1, 0, FALSE, FALSE, 12,
       'ALL', NULL, 'SATURDAY,SUNDAY', NULL, NULL, NULL, NULL, 'Cuối tuần', '/search',
...
-- Mã giới hạn provider demo: giảm 10.000đ cho provider mẫu.
INSERT INTO promo_codes (
    code, name, description, owner_type, provider_id, created_by_user_id,
    promotion_type, target_type, discount_type, discount_value, max_discount_amount, min_order_amount,
    usage_limit_total, usage_limit_per_user, usage_count, is_stackable, is_auto_apply, priority,
...
SELECT 'PROVIDER10K', 'Provider mẫu giảm 10.000đ',
       'Mã demo admin giới hạn cho PetGo Sample Provider.',
       'ADMIN', NULL, @sample_admin_id, 'PROMO_CODE', 'BOOKING', 'FIXED_AMOUNT', 10000.00, 10000.00, 0.00,
       200, 1, 0, FALSE, FALSE, 12,
       'ALL', NULL, NULL, CAST(@sample_provider_id AS CHAR), NULL, NULL, NULL, 'Provider deal', '/providers',
       'Chỉ áp dụng cho provider demo nếu provider tồn tại.',
       'Seed demo admin provider-scoped promotion.', NOW(), DATE_ADD(NOW(), INTERVAL 90 DAY), TRUE
WHERE @sample_admin_id IS NOT NULL
  AND @sample_provider_id IS NOT NULL
  AND NOT EXISTS (SELECT 1 FROM promo_codes WHERE code = 'PROVIDER10K');

-- Mã giới hạn dịch vụ demo: giảm 10.000đ cho service cụ thể nếu tồn tại.
INSERT INTO promo_codes (
    code, name, description, owner_type, provider_id, created_by_user_id,
    promotion_type, target_type, discount_type, discount_value, max_discount_amount, min_order_amount,
    usage_limit_total, usage_limit_per_user, usage_count, is_stackable, is_auto_apply, priority,
...
SELECT 'SERVICE10K', 'Dịch vụ mẫu giảm 10.000đ',
       'Mã demo admin giới hạn cho một dịch vụ provider cụ thể.',
       'ADMIN', NULL, @sample_admin_id, 'PROMO_CODE', 'BOOKING', 'FIXED_AMOUNT', 10000.00, 10000.00, 0.00,
       200, 1, 0, FALSE, FALSE, 12,
       'ALL', NULL, NULL, NULL, CAST(@sample_bath_service_id AS CHAR), NULL, NULL, 'Dịch vụ', '/search',
       'Chỉ áp dụng cho dịch vụ mẫu nếu dữ liệu dịch vụ tồn tại.',
       'Seed demo admin service-scoped promotion.', NOW(), DATE_ADD(NOW(), INTERVAL 90 DAY), TRUE
WHERE @sample_admin_id IS NOT NULL
  AND @sample_bath_service_id IS NOT NULL
  AND NOT EXISTS (SELECT 1 FROM promo_codes WHERE code = 'SERVICE10K');
...
SELECT 'SPA10K', 'Spa thú cưng giảm 10.000đ',
       ...
       'ADMIN', NULL, @sample_admin_id, 'PROMO_CODE', 'BOOKING', 'FIXED_AMOUNT', 10000.00, 10000.00, 0.00,
       300, 1, 0, FALSE, FALSE, 12,
       'ALL', NULL, NULL, NULL, NULL, CAST(@sample_grooming_category_id AS CHAR), NULL, 'Spa deal', '/search',
...
SELECT 'MEMBER10K', 'Membership giảm 10.000đ',
       ...
       'ADMIN', NULL, @sample_admin_id, 'MEMBERSHIP', 'MEMBERSHIP', 'FIXED_AMOUNT', 10000.00, 10000.00, 0.00,
       200, 1, 0, FALSE, FALSE, 12,
       'ALL', NULL, NULL, NULL, NULL, NULL, NULL, 'Hội viên', '/membership',
...
WHERE code IN (
    ...
    'WEEKEND10K', 'PROVIDER10K', 'SERVICE10K', 'SPA10K', 'MEMBER10K'
)
